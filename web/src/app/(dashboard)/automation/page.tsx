...
import {
    ...
    Button, Input, Textarea, Toggle, Checkbox, Chip, Badge,
                    ...
                    <div className="layout-split-main flex flex-col gap-6">
                        ...
                        <Card>
                                ...
                                <div className="flex items-center gap-3">
                                    <div
                                        style={{
                                            ...
                                            background: defaultRule.is_active ? 'var(--success)' : 'var(--color-gray-300)',
                                            boxShadow: defaultRule.is_active ? '0 0 8px var(--success)' : 'none'
                                        }}
                                    />
                                    <CardTitle subtitle="Applies to all your posts">
                                        Default Auto-Reply
                                    </CardTitle>
                                </div>
                                <Toggle
                                    checked={defaultRule.is_active}
                                    onChange={(checked) => setDefaultRule({ ...defaultRule, is_active: checked })}
                                />
                            </CardHeader>
                            ...
                            <CardBody>
                                ...
                                <div className="mb-5">
                                    ...
                                    >
                                        ...
                                        {defaultRule.keywords.length > 0 && (
                                            <Badge variant="primary" pill>{defaultRule.keywords.length}</Badge>
                                        )}
                                    </button>
                                        ...
                                        <div style={{ marginTop: 'var(--space-4)', paddingLeft: 'var(--space-6)' }}>
                                            ...
                                            <div className="flex gap-3">
                                                <input
                                                    ...
                                                    onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), addKeyword(defaultKwInput, setDefaultKwInput, defaultRule, setDefaultRule))}
                                                ...
                                                <Button
                                                    variant="secondary"
                                                    onClick={() => addKeyword(defaultKwInput, setDefaultKwInput, defaultRule, setDefaultRule)}
                                                >
                                                    Add
                                                </Button>
                                            </div>
                                            {defaultRule.keywords.length > 0 && (
                                                <div className="flex flex-wrap gap-2 mt-3">
                                                    {defaultRule.keywords.map(k => (
                                                        <Chip key={k} onRemove={() => setDefaultRule({ ...defaultRule, keywords: defaultRule.keywords.filter(x => x !== k) })}>
                                                            {k}
                                                        </Chip>
                                                    ))}
                                                </div>
                                ...
                                <div className="mb-6">
                                    <Checkbox
                                        checked={defaultRule.send_dm}
                                        onChange={(checked) => setDefaultRule({ ...defaultRule, send_dm: checked })}
                                        label="Also send a private DM"
                                    />
                        ...
                        <Card>
                            <CardHeader>
                                <CardTitle subtitle="Custom replies for specific posts">
                                    Post Overrides
                                </CardTitle>
                                <Button
                                    variant={showCreateOverride ? "ghost" : "secondary"}
                                    size="sm"
                                ...
                                <div style={{ padding: 'var(--space-6)', background: 'var(--color-gray-50)', borderBottom: '1px solid var(--border-light)' }}>
                                    ...
                                    <div className="mb-5">
                                        <label className="form-label">
                                            Select posts {(newRule.media_ids?.length || 0) > 0 && <span className="text-primary">({newRule.media_ids?.length} selected)</span>}
                                        </label>
                                        <div
                                            style={{
                                                display: 'grid',
                                        ...
                                        >
                                            {media.length === 0 ? (
                                                <p className="text-muted text-center" style={{ gridColumn: '1/-1', padding: 'var(--space-6)' }}>No posts available</p>
                                            ) : (
                                                media.map(m => {
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }} className="mb-5">
                                        ...
                                        <div>
                                            <label className="form-label">Private DM</label>
                                            <textarea
                                    ...
                                    <Button variant="primary" block onClick={createOverride} isLoading={saving}>
                                        Create override
                                    </Button>
                                </div>
                            )}
                            ...
                            <div>
                                {specificRules.length === 0 ? (
                                    <EmptyState
                                        ...
                                        description="Create custom replies for specific posts"
                                    />
                                ) : (
                                    specificRules.map(rule => {
                                        ...
                                        return (
                                            <div key={rule.id} style={{ borderBottom: '1px solid var(--border-light)' }}>
                                                <div
                                                    onClick={() => setExpandedOverride(expanded ? null : rule.id || null)}
                                                ...
                                                {expanded && (
                                                    <div style={{ padding: 'var(--space-4) var(--space-6)', background: 'var(--color-gray-50)', borderTop: '1px solid var(--border-light)' }}>
                                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }} className="mb-4">
                                                            <div>
                                                                <span className="text-sm text-muted font-semibold" style={{ textTransform: 'uppercase', fontSize: 'var(--text-xs)' }}>Comment Reply</span>
                                                                <p style={{ marginTop: 'var(--space-2)' }}>{rule.comment_reply}</p>
                                                            </div>
                                                            {rule.send_dm && (
                                                                <div>
                                                                    <span className="text-sm text-muted font-semibold" style={{ textTransform: 'uppercase', fontSize: 'var(--text-xs)' }}>Private DM</span>
                                                                    <p style={{ marginTop: 'var(--space-2)' }}>{rule.dm_reply}</p>
                                                                </div>
                                                            )}
                                                        </div>
                        ...
                        <Card>
                            <CardHeader style={{ background: 'linear-gradient(135deg, var(--color-primary-50) 0%, var(--color-primary-100) 100%)' }}>
                                <div className="flex items-center gap-2">
                                    ...
                                    <CardTitle subtitle="How your auto-reply will appear">
                                        <span style={{ color: 'var(--color-primary-700)' }}>Live Preview</span>
                                    </CardTitle>
                                </div>
                            </CardHeader>
                            ...
                            <CardBody>
                                {/* Incoming Comment */}
                                <div className="mb-4">
                                    <span style={{ fontSize: 'var(--text-xs)', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--muted)', fontWeight: 600 }}>
                                        Incoming Comment
                                    </span>
                                    <div className="flex items-start gap-3 mt-3">
                                        ...
                                        >
                                            ...
                                            <p style={{ marginTop: 'var(--space-1)' }}>
                                                {defaultRule.keywords.length > 0 ? (
                                                    <>Hey, what's the <span style={{ background: 'var(--warning-light)', padding: '1px 4px', borderRadius: 4 }}>{defaultRule.keywords[0]}</span>?</>
                                    ...
                                    <div className="flex items-start gap-3 mt-3">
                                        <div
                                            style={{
                                                ...
                                                borderRadius: 'var(--radius-full)',
                                                flexShrink: 0,
                                                background: 'linear-gradient(135deg, var(--primary) 0%, var(--color-primary-600) 100%)'
                                            }}
                                        />
                                        <div
                                            style={{
                                        ...
                                        >
                                            ...
                                            <p style={{ marginTop: 'var(--space-1)' }}>
                                                {defaultRule.comment_reply || <span className="text-muted" style={{ fontStyle: 'italic' }}>Enter a reply above...</span>}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {defaultRule.send_dm && (
                                    <>
                                        ...
                                        >
                                            ...
                                            <p>
                                                {defaultRule.dm_reply || <span className="text-muted" style={{ fontStyle: 'italic' }}>Enter a DM message...</span>}
                                            </p>
                                        </div>
                                    </>
                                )}
                                ...
                                <div
                                    ...
                                    style={{
                                        padding: 'var(--space-3) var(--space-4)',
                                        borderRadius: 'var(--radius-md)',
                                        background: defaultRule.is_active ? 'var(--success-light)' : 'var(--color-gray-100)'
                                    }}
                                >
                                    <div
                                        style={{
                                            ...
                                            height: 8,
                                            borderRadius: 'var(--radius-full)',
                                            background: defaultRule.is_active ? 'var(--success)' : 'var(--muted)',
                                            boxShadow: defaultRule.is_active ? '0 0 6px var(--success)' : 'none'
                                        }}
                                    />
